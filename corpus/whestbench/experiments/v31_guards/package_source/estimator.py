"""Kerdock v3.1: frozen v3 plus the predeclared guard mutations M186/M187.

Governing evidence: a4_results.json + A3_A4_NOTES.md (experiments/
a_series_granular_adversarial).  All frozen v3 sources ship as byte-identical
copies (the frozen ``estimator.py`` rides along as ``kerdock_v3_estimator.py``,
sha256 076d0a5d...); this file only subclasses and adds the two guards.
...
Healthy-net contract (gate G1): the frozen path runs unchanged and its output
object is returned untouched, so predictions are bitwise identical to frozen
v3.  The only added billed work on a healthy net is one isfinite scan plus
its reduction over the (depth, width) output stack, billed through flopscope
like every other op (~2e4 FLOPs against ~1.8e11, i.e. ~1e-5 percent).
...
class Estimator(_FrozenKerdockV3):
    """Frozen Kerdock v3 with the M186 empty-regime and M187 finite guards."""
