"""Isolated, truth-free, restricted-import M145 deployment descendant.

This module deliberately does not replace the frozen Formal-L1 champion.  It
imports that champion's hash-bound analytic/folding primitives, generates a
positive-diagonal-Haar provisional radius-scaled QR bank for a matched
...
from m145_deployable_core import (  # noqa: E402
    DIMENSION,
    MAIN_FRAMES,
    PILOT_FRAMES,
    PILOT_LINES,
    ...
    TOTAL_FRAMES,
    predict_child_seeds,
    setup_child_seeds as _setup_child_seeds,
)
from m145_deployable_sidecar import (  # noqa: E402
    _draw_main_anchors,
...
def setup_child_seeds(setup_seed: int) -> tuple[int, int]:
    return _setup_child_seeds(int(setup_seed))


def _sign_correct_haar_q(q, r):
    """Return the positive-diagonal QR representative of a Gaussian matrix.

    For an iid Gaussian square matrix ``G=QR``, multiplication by the diagonal
    signs of ``R`` produces ``Q D`` and ``D R`` with nonnegative diagonal.  The
    resulting Q is Haar on O(d); unlike raw LAPACK QR, its oriented rows have
    the required invariant law.  This operation is visible to FlopScope.
    """

    # Avoid a host-side or optional diagonal helper: each slice remains a
    # FlopScope-tracked array operation and the width is frozen at 256.
    diagonal = fnp.stack(tuple(r[:, index, index] for index in range(DIMENSION)), axis=1)
    signs = fnp.where(diagonal < fnp.float32(0.0), fnp.float32(-1.0), fnp.float32(1.0))
    return fnp.multiply(q, signs[:, None, :]).astype(fnp.float32)


def signed_haar_radius_bank(
    setup_seed: int,
    *,
    width: int = DIMENSION,
    ...
    """Construct the exact positive-diagonal Haar bank used in deployment.

    This deliberately uses only FlopScope operations, so the structural
    comparator/reference sees the same target construction as production.
    """

    if int(width) != DIMENSION:
        raise ValueError("deployable M145 bank is frozen at width 256")
    pilot_seed, main_seed = setup_child_seeds(int(setup_seed))
    pilot_rng = fnp.random.default_rng(pilot_seed)
    main_rng = fnp.random.default_rng(main_seed)
    pilot_raw = pilot_rng.standard_normal(
        (int(pilot_frames), DIMENSION, DIMENSION), dtype=fnp.float32
    )
    main_raw = main_rng.standard_normal(
        (int(main_frames), DIMENSION, DIMENSION), dtype=fnp.float32
    )
    pilot_q, pilot_r = fnp.linalg.qr(pilot_raw)
    main_q, main_r = fnp.linalg.qr(main_raw)
    bank = fnp.concatenate(
        (_sign_correct_haar_q(pilot_q, pilot_r), _sign_correct_haar_q(main_q, main_r)),
        axis=0,
    ).astype(fnp.float32)
    return fnp.multiply(bank, fnp.float32(mean_radius(DIMENSION)))
...
class Estimator(_FormalFold3Estimator):
    ...
    def __init__(self) -> None:
        super().__init__()
        self._setup_seed = 0
        self._mean_radius = fnp.float32(mean_radius(DIMENSION))
    ...
    def setup(self, ctx) -> None:
        ...
        bank = signed_haar_radius_bank(self._setup_seed)
        ...
        # ``signed_haar_radius_bank`` owns the QR temporaries. They have already
        # left scope before this method receives ``bank``; attempting to delete
        # those names here makes the deployable estimator fail during setup.
    ...
    def predict(self, mlp, budget):
        ...
        if self.transport_enabled:
            ...
            child_seeds = predict_child_seeds(self._setup_seed, int(mlp.seed))
