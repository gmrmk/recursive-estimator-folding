"""M240: meter-safe finite-scan child of M238's live M179 event packer.
...
class PersistentEventPacker:
    ...
    def __init__(self, layers: int, width: int, events_per_layer: int):
        ...
        self._derived_finite = self._bool_block[: len(_WORK_NAMES)]
        self._output_finite = self._bool_block[: len(DERIVED_COLUMN_NAMES)]
        self._raw_finite_base = np.asarray(self._raw_finite)
        self._derived_finite_base = np.asarray(self._derived_finite)
        self._output_finite_base = np.asarray(self._output_finite)
    ...
    def _require_raw_interior(self) -> None:
        x = self._raw
        fnp.isfinite(self._raw_block, out=self._raw_finite_base)
    ...
    def _require_derived_finite(self) -> None:
        ...
        fnp.isfinite(self._work_block, out=self._derived_finite_base)
    ...
    def _commit_outputs(self, receipt: StrictEventReceipt) -> PackedColumns:
        ...
        fnp.isfinite(self._output_block, out=self._output_finite_base)
