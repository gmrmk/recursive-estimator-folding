"""One-shot durable transport for M241's isolated six-test G0A replay."""
...
INTENT = HERE / "M241_G0A_LAUNCH_INTENT_20260809.json"
RESULT = HERE / "M241_G0A_RESULT_20260809.json"
TEMP = HERE / ".M241_G0A_RESULT_20260809.json.tmp"
TIMEOUT_S = 120
...
PARENTS = {
    ...
    M240 / "run_m240_g0a_receipt.py":
        "1954EDAA57D4B99B073A63706587AD58D98AE983AD87A917A8770BFC535125B1",
    M240 / "test_m240_g0a_runner_static.py":
        "5C283B1D9F7BEB3D31A929B892DEC617BC290BBF70CB5274EDBC6E920B05F307",
    M240 / "M240_G0A_LAUNCH_INTENT_20260809.json":
        "C8EB887E05BE8F3B08EED16B19FCDBCF49B6041CA2A13F6127EB59BE406CA484",
    M240 / "M240_G0A_RESULT_20260809.json":
        "456B8E6897474BC48112BC2671A46B180EA28AE150700614FBD6DDD0F6202FF1",
    M240 / "M240_DISPOSITION_20260809.md":
...
def verify_parents() -> dict[str, str]:
    ...
    if bad:
        raise RuntimeError("M241 frozen parent mismatch: " + json.dumps(bad, sort_keys=True))
    return observed
...
def main() -> int:
    if INTENT.exists() or RESULT.exists() or TEMP.exists():
        raise FileExistsError("M241 launch/result path already exists")
    if FORBIDDEN_METHOD in COMMAND:
        raise RuntimeError("M241 command contains forbidden G0B method")
    if len(METHODS) != 6 or len(COMMAND) != 10:
        raise RuntimeError("M241 exact six-test command mismatch")
    ...
    intent_payload = {
        "mutation": "m241_isolated_g0a_fixtures",
        "cwd": str(M241),
