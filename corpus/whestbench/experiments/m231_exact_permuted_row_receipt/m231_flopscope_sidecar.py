"""FlopScope 0.10 M231 exact-permutation receipt and row-HT circuit."""

from __future__ import annotations

from dataclasses import dataclass
from pathlib import Path
import sys

import flopscope.numpy as fnp
import numpy as np


HERE = Path(__file__).resolve().parent
BASE = HERE.parent
for path in (
    HERE,
    BASE / "m212_backend_packed_explicit_symmetry",
    BASE / "m215_rankone_collision_correction",
    BASE / "m227_row_subset_collision_ht",
):
    if str(path) not in sys.path:
        sys.path.insert(0, str(path))

from m215_flopscope_sidecar import (  # noqa: E402
    FullDomainReceipt,
    issue_full_domain_receipt,
)


WIDTH = 256
LAYERS = 31
SUBSET_ROWS = 32
DOMAIN = "M231_ROW_PERMUTED_V1"
DOMAIN_XOR = 0x231A011
DTYPE = fnp.float64


@dataclass
class RowWorkspace:
    powers: object
    cross: object
    subset_rows: int
    selected_values: object | None = None
    rank_receipt: object | None = None


@dataclass(frozen=True)
class PermutedRowReceipt:
    layer_ids: tuple[int, ...]
    producer_epoch: int
    domain: str
    width: int
    subset_rows: int
    rank_order: object
    selected: object


def issue_permuted_receipt(
    *,
    layers: int,
    width: int,
    subset_rows: int,
    seed: int,
    producer_epoch: int,
    domain: str,
) -> PermutedRowReceipt:
    """Issue one independently permuted integer-label row per layer."""

    layer_count = int(layers)
    row_count = int(width)
    k = int(subset_rows)
    if layer_count < 1 or row_count < 3 or k < 1 or k >= row_count:
        raise ValueError("invalid M231 receipt dimensions")
    if str(domain) != DOMAIN:
        raise ValueError("M231 random domain mismatch")
    generator = fnp.random.default_rng(int(seed) ^ DOMAIN_XOR)
    label_base = fnp.arange(row_count, dtype=fnp.int64)
    label_bank = fnp.broadcast_to(label_base, (layer_count, row_count))
    rank_order = generator.permuted(label_bank, axis=1)
    selected = rank_order[:, :k]
    return PermutedRowReceipt(
        layer_ids=tuple(range(1, layer_count + 1)),
        producer_epoch=int(producer_epoch),
        domain=DOMAIN,
        width=row_count,
        subset_rows=k,
        rank_order=rank_order,
        selected=selected,
    )


def validate_receipt(
    receipt: PermutedRowReceipt,
    *,
    layers: int,
    width: int,
    subset_rows: int,
    producer_epoch: int,
    domain: str,
) -> None:
    expected_ids = tuple(range(1, int(layers) + 1))
    if receipt.layer_ids != expected_ids:
        raise ValueError("M231 receipt layer binding mismatch")
    if int(receipt.producer_epoch) != int(producer_epoch):
        raise ValueError("M231 receipt producer epoch mismatch")
    if receipt.domain != DOMAIN or str(domain) != DOMAIN:
        raise ValueError("M231 receipt random domain mismatch")
    if int(receipt.width) != int(width) or int(receipt.subset_rows) != int(subset_rows):
        raise ValueError("M231 receipt shape metadata mismatch")
    if tuple(receipt.rank_order.shape) != (int(layers), int(width)):
        raise ValueError("M231 rank receipt shape mismatch")
    if tuple(receipt.selected.shape) != (int(layers), int(subset_rows)):
        raise ValueError("M231 selected receipt shape mismatch")
    if str(receipt.rank_order.dtype) != "int64" or str(receipt.selected.dtype) != "int64":
        raise ValueError("M231 receipt dtype mismatch")


def co_permute_receipt(
    receipt: PermutedRowReceipt, permutation: np.ndarray
) -> PermutedRowReceipt:
    """Test-only pathwise action on a frozen receipt, without RNG replay."""

    order = np.asarray(permutation, dtype=np.int64)
    width = int(receipt.width)
    if order.shape != (width,) or not np.array_equal(np.sort(order), np.arange(width)):
        raise ValueError("invalid hidden-label permutation")
    inverse = np.empty(width, dtype=np.int64)
    inverse[order] = np.arange(width, dtype=np.int64)
    rank = inverse[np.asarray(receipt.rank_order)]
    selected = rank[:, : receipt.subset_rows]
    return PermutedRowReceipt(
        layer_ids=receipt.layer_ids,
        producer_epoch=receipt.producer_epoch,
        domain=receipt.domain,
        width=receipt.width,
        subset_rows=receipt.subset_rows,
        rank_order=rank,
        selected=selected,
    )


def allocate_row_workspace(
    layers: int = LAYERS,
    width: int = WIDTH,
    subset_rows: int = SUBSET_ROWS,
) -> RowWorkspace:
    k = int(subset_rows)
    if layers < 1 or width < 3 or k < 1 or k >= width:
        raise ValueError("invalid M231 workspace dimensions")
    return RowWorkspace(
        powers=fnp.empty((2, layers, k, width), dtype=DTYPE),
        cross=fnp.empty((2, layers, width, width), dtype=DTYPE),
        subset_rows=k,
    )


def allocation_ledger(staged, base_workspace, row_workspace: RowWorkspace) -> dict[str, object]:
    layers, width = int(staged.weight.shape[0]), int(staged.weight.shape[-1])
    k = int(row_workspace.subset_rows)
    arrays = {
        "weight": staged.weight,
        "factor": staged.factor,
        "scaled_S": base_workspace.scaled,
        "live_B_then_Dhat": base_workspace.gram,
        "p": base_workspace.p,
        "p2": base_workspace.p2,
        "rho_then_that": base_workspace.rho,
        "rho_p": base_workspace.rho_p,
        "scratch": base_workspace.scratch,
        "strict_aaab": base_workspace.aaab,
        "strict_aabb": base_workspace.aabb,
        "strict_aaaa": base_workspace.aaaa,
        "selected_powers_S2_S3": row_workspace.powers,
        "cross_Ahat_Ehat": row_workspace.cross,
    }
    for index, value in enumerate(base_workspace.level_products):
        arrays[f"M212_level_product_{index}"] = value
    items = {
        name: {
            "shape": list(value.shape),
            "dtype": str(value.dtype),
            "elements": int(value.size),
            "bytes": int(value.nbytes),
        }
        for name, value in arrays.items()
    }
    m231_names = {"selected_powers_S2_S3", "cross_Ahat_Ehat"}
    m212_bytes = sum(
        item["bytes"] for name, item in items.items() if name not in m231_names
    )
    selected_bytes = layers * k * width * 8
    rank_bytes = layers * width * 8
    incremental_bytes = (
        int(row_workspace.powers.nbytes)
        + int(row_workspace.cross.nbytes)
        + selected_bytes
        + rank_bytes
    )
    label_base_bytes = width * 8
    return {
        "arrays": items,
        "selected_values_bytes": selected_bytes,
        "rank_receipt_bytes": rank_bytes,
        "transient_label_base_bytes": label_base_bytes,
        "incremental_persistent_bytes": incremental_bytes,
        "incremental_persistent_mib": incremental_bytes / (1024.0 * 1024.0),
        "incremental_nominal_peak_mib": (incremental_bytes + label_base_bytes)
        / (1024.0 * 1024.0),
        "m212_m231_persistent_mib": (m212_bytes + incremental_bytes)
        / (1024.0 * 1024.0),
        "rank3_coefficient_arrays": 0,
        "sampled_Bhat_or_rhohat_arrays": 0,
    }


def _validate_live_binding(
    staged,
    workspace,
    row_workspace: RowWorkspace,
    receipt: FullDomainReceipt,
    subset_rows: int,
) -> tuple[int, int, int]:
    layers, width = int(staged.weight.shape[0]), int(staged.weight.shape[-1])
    k = int(subset_rows)
    expected_ids = tuple(range(1, layers + 1))
    if staged.layer_ids != expected_ids or receipt.layer_ids != expected_ids:
        raise ValueError("M231 requires canonical layer order")
    if staged.producer_epoch is None or int(staged.producer_epoch) != int(receipt.producer_epoch):
        raise ValueError("M231 producer epoch mismatch")
    if k != int(row_workspace.subset_rows) or k < 1 or k >= width:
        raise ValueError("M231 subset/workspace mismatch")
    live = (workspace.aaaa, workspace.aaab, workspace.aabb, workspace.gram, workspace.p)
    bound = (receipt.aaaa, receipt.aaab, receipt.aabb, receipt.gram, receipt.p)
    if any(left is not right for left, right in zip(live, bound, strict=True)):
        raise ValueError("M231 receipt no longer owns live M212 objects")
    if tuple(row_workspace.powers.shape) != (2, layers, k, width):
        raise ValueError("M231 powers workspace shape mismatch")
    if tuple(row_workspace.cross.shape) != (2, layers, width, width):
        raise ValueError("M231 cross workspace shape mismatch")
    values = (
        staged.weight,
        staged.factor,
        workspace.scaled,
        workspace.gram,
        workspace.p,
        workspace.rho,
        workspace.scratch,
        workspace.aaab,
        workspace.aabb,
        workspace.aaaa,
        row_workspace.powers,
        row_workspace.cross,
    )
    if any(str(value.dtype) != "float64" for value in values):
        raise ValueError("M231 requires a fully float64 live stack")
    return layers, width, k


def subtract_permuted_row_sketch_inplace(
    staged,
    workspace,
    row_workspace: RowWorkspace,
    receipt: FullDomainReceipt,
    *,
    seed: int,
    subset_rows: int = SUBSET_ROWS,
    domain: str = DOMAIN,
):
    layers, width, k = _validate_live_binding(
        staged, workspace, row_workspace, receipt, subset_rows
    )
    x = workspace
    r = row_workspace
    row_receipt = issue_permuted_receipt(
        layers=layers,
        width=width,
        subset_rows=k,
        seed=int(seed),
        producer_epoch=int(staged.producer_epoch),
        domain=domain,
    )
    validate_receipt(
        row_receipt,
        layers=layers,
        width=width,
        subset_rows=k,
        producer_epoch=int(staged.producer_epoch),
        domain=domain,
    )
    selected = fnp.take_along_axis(
        x.scaled, row_receipt.selected[:, :, None], axis=1
    )
    r.selected_values = selected
    r.rank_receipt = row_receipt.rank_order

    fnp.multiply(selected, selected, out=r.powers[0])
    fnp.multiply(r.powers[0], selected, out=r.powers[1])
    fnp.matmul(fnp.transpose(r.powers, (0, 1, 3, 2)), selected, out=r.cross)

    fnp.multiply(x.rho[:, :, None], x.gram, out=x.scratch)
    fnp.multiply(x.scratch, fnp.float64(12.0), out=x.scratch)
    fnp.add(x.aaab, x.scratch, out=x.aaab)
    fnp.multiply(x.gram, x.gram, out=x.scratch)
    fnp.multiply(x.scratch, fnp.float64(8.0), out=x.scratch)
    fnp.add(x.aabb, x.scratch, out=x.aabb)
    fnp.multiply(x.rho[:, :, None], x.rho[:, None, :], out=x.scratch)
    fnp.multiply(x.scratch, fnp.float64(4.0), out=x.scratch)
    fnp.add(x.aabb, x.scratch, out=x.aabb)

    scale = float(width) / float(k)
    fnp.multiply(x.p[:, :, None], r.cross[0], out=x.scratch)
    fnp.multiply(x.scratch, fnp.float64(18.0 * scale), out=x.scratch)
    fnp.add(x.aaab, x.scratch, out=x.aaab)
    fnp.multiply(r.cross[0], x.p[:, None, :], out=x.scratch)
    fnp.multiply(x.scratch, fnp.float64(12.0 * scale), out=x.scratch)
    fnp.add(x.aabb, x.scratch, out=x.aabb)
    fnp.add(x.aabb, fnp.swapaxes(x.scratch, 1, 2), out=x.aabb)

    fnp.sum(r.powers[1], axis=1, out=x.rho)
    fnp.multiply(x.rho[:, :, None], x.p[:, None, :], out=x.scratch)
    fnp.multiply(x.scratch, fnp.float64(6.0 * scale), out=x.scratch)
    fnp.add(x.aaab, x.scratch, out=x.aaab)
    fnp.multiply(r.cross[1], fnp.float64(-24.0 * scale), out=r.cross[1])
    fnp.add(x.aaab, r.cross[1], out=x.aaab)

    fnp.matmul(
        fnp.transpose(r.powers[0], (0, 2, 1)),
        r.powers[0],
        out=x.gram,
    )
    fnp.multiply(x.gram, fnp.float64(-24.0 * scale), out=x.gram)
    fnp.add(x.aabb, x.gram, out=x.aabb)
    fnp.copyto(x.aaaa, fnp.diagonal(x.aaab, axis1=1, axis2=2))
    return (x.aaaa, x.aaab, x.aabb), row_receipt


__all__ = [
    "DOMAIN",
    "FullDomainReceipt",
    "PermutedRowReceipt",
    "RowWorkspace",
    "allocate_row_workspace",
    "allocation_ledger",
    "co_permute_receipt",
    "issue_full_domain_receipt",
    "issue_permuted_receipt",
    "subtract_permuted_row_sketch_inplace",
    "validate_receipt",
]
